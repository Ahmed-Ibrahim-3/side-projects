...
def main():
    ...
    pygame.display.set_caption('wish.com Bejeweled ')
        ...
        if clickedSpace and not firstSelectedGem:
            ...
            if matchedGems == []:
                ...
                while matchedGems != []:
                    ...
                    # points is a list of dicts that tells fillBoardAndAnimate() where on the screen to display text to show how many
                    # points the player got. points is a list because if the playergets multiple matches, then multiple points text should appear.
                    points = []
...
def getSwappingGems(board, firstXY, secondXY):
    # If the gems at the (X, Y) coordinates of the two gems are adjacent, then their 'direction' keys are set to the appropriate direction
    # value to be swapped with each other. Otherwise, (None, None) is returned.
...
def canMakeMove(board):
    ...
    # The patterns in oneOffPatterns represent gems that are configured in a way where it only takes one move to make a triplet.
...
def drawMovingGem(gem, progress):
    # Draw a gem sliding in the direction that its 'direction' key indicates. The progress parameter is a number from 0 (just starting) to 100 (slide complete).
...
def getDropSlots(board):
    # Creates a "drop slot" for each column and fills the slot with a number of gems that that column is lacking. This function assumes that the gems have been gravity dropped already.
...
def fillBoardAndAnimate(board, points, score):
    ...
    while dropSlots != [[]] * BOARDWIDTH:
        ...
        # Make the next row of gems from the drop slots the lowest by deleting the previous lowest gems.
        for x in range(len(dropSlots)):
            if len(dropSlots[x]) == 0:
                continue
...
def getBoardCopyMinusGems(board, gems):
    # Creates and returns a copy of the passed board data structure, with the gems in the "gems" list removed from it. Gems is a list of dicts, with keys x, y, direction, imageNum
